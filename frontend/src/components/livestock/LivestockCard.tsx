...
export default function LivestockCard({
  ...
  const getTypeColor = () => {
    ...
    switch (livestock.type) {
      case 'fish': return 'border-blue-300 bg-blue-50'
      case 'coral': return 'border-purple-300 bg-purple-50'
      case 'invertebrate': return 'border-orange-300 bg-orange-50'
  ...
  return (
    <div className={`rounded-lg shadow border-2 ${getTypeColor()} overflow-hidden ${isPast ? 'opacity-75' : ''} ${livestock.is_archived ? 'opacity-60' : ''}`}>
      ...
      <div className="flex items-start p-3 gap-3">
        {/* Thumbnail - smaller */}
        {hasThumbnail ? (
          <div className={`w-16 h-16 flex-shrink-0 rounded-md overflow-hidden bg-gradient-to-b from-blue-100 to-blue-50 ${isPast ? 'grayscale' : ''}`}>
            {imageLoading ? (
          ...
          <div className="w-16 h-16 flex-shrink-0 rounded-md flex items-center justify-center bg-white/50">
        ...
        <div className="flex-1 min-w-0">
          ...
          {livestock.common_name && (
            <p className="text-xs text-gray-500 italic truncate">{livestock.species_name}</p>
          )}
        ...
        <div className="flex space-x-0.5 flex-shrink-0">
          ...
          {livestock.is_archived ? (
            <button onClick={() => onUnarchive(livestock.id)} className="p-1 text-green-600 hover:bg-green-100 rounded" title={tc('actions.unarchive')}>
              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <button onClick={() => onArchive(livestock.id)} className="p-1 text-gray-500 hover:bg-gray-100 rounded" title={tc('actions.archive')}>
              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4" />
              </svg>
            </button>
          )}
          <button onClick={() => onEdit(livestock)} className="p-1 text-gray-600 hover:bg-gray-200 rounded" title={tc('actions.edit')}>
            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
            </svg>
          </button>
          <button onClick={() => onDelete(livestock.id)} className="p-1 text-red-600 hover:bg-red-100 rounded" title={tc('actions.delete')}>
            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="px-3 pb-2 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-gray-600">
        ...
        {livestock.purchase_price && (
          <span className="inline-flex items-center gap-0.5" title={livestock.quantity > 1 ? (() => {
            ...
            <span className="font-medium text-gray-800">{livestock.purchase_price}</span>
            {livestock.quantity > 1 && (() => {
              const unitPrice = parsePrice(livestock.purchase_price)
        ...
        {timeInTank && (
          <span className="inline-flex items-center gap-0.5">
            <span className="text-gray-400">🕐</span>
            <span className={`font-medium ${isPast ? 'text-gray-500' : 'text-green-700'}`}>{timeInTank}</span>
          </span>
        )}
        ...
        <div className="px-3 pb-2 flex flex-wrap gap-2">
          {livestock.worms_id && (
            <a href={`https://www.marinespecies.org/aphia.php?p=taxdetails&id=${livestock.worms_id}`}
              ...
              className="inline-flex items-center px-1.5 py-0.5 rounded bg-blue-50 text-[10px] text-blue-700 hover:bg-blue-100 transition-colors"
              onClick={(e) => e.stopPropagation()}>
              WoRMS
            </a>
          )}
          {livestock.inaturalist_id && (
            <a href={`https://www.inaturalist.org/taxa/${livestock.inaturalist_id}`}
              target="_blank" rel="noopener noreferrer"
              className="inline-flex items-center px-1.5 py-0.5 rounded bg-green-50 text-[10px] text-green-700 hover:bg-green-100 transition-colors"
              onClick={(e) => e.stopPropagation()}>
              iNaturalist
            </a>
          )}
          {livestock.fishbase_species_id && (
            <a href={`https://www.fishbase.se/summary/${livestock.fishbase_species_id}`}
              target="_blank" rel="noopener noreferrer"
              className="inline-flex items-center px-1.5 py-0.5 rounded bg-cyan-50 text-[10px] text-cyan-700 hover:bg-cyan-100 transition-colors"
              onClick={(e) => e.stopPropagation()}>
              FishBase
            </a>
          )}
      ...
      {livestock.notes && (
        <div className="px-3 pb-2">
          <p className="text-[11px] text-gray-500 line-clamp-2 italic">{livestock.notes}</p>
        </div>
      )}

      {/* Inline Split Dialog */}
      {showSplitDialog && (
        <div className="p-3 bg-amber-50 border-t-2 border-amber-300">
          <h4 className="text-xs font-semibold text-gray-900 mb-1">{t('split.title')}</h4>
          ...
          <div className="mb-2">
            <div className="flex items-center gap-2">
              <input
                ...
                className="w-16 px-2 py-1 text-xs border border-gray-300 rounded-md focus:ring-2 focus:ring-amber-500 focus:border-amber-500"
              />
              <span className="text-[10px] text-gray-500">{t('split.remaining', { count: livestock.quantity - splitQuantity })}</span>
            </div>
          </div>
          <div className="flex items-center gap-2 mb-2">
            <button type="button" onClick={() => setSplitStatus('dead')}
              className={`px-2 py-0.5 text-[10px] rounded-md border-2 transition-colors ${splitStatus === 'dead' ? 'border-red-500 bg-red-50 text-red-800' : 'border-gray-300 text-gray-700 hover:border-gray-400'}`}>
              {t('status.dead')}
            </button>
            <button type="button" onClick={() => setSplitStatus('removed')}
              className={`px-2 py-0.5 text-[10px] rounded-md border-2 transition-colors ${splitStatus === 'removed' ? 'border-yellow-500 bg-yellow-50 text-yellow-800' : 'border-gray-300 text-gray-700 hover:border-gray-400'}`}>
              {t('status.removed')}
            </button>
            <div className="flex-1" />
            <button type="button" onClick={() => setShowSplitDialog(false)}
              className="px-2 py-0.5 text-[10px] text-gray-700 border border-gray-300 rounded-md hover:bg-gray-50">
              {tc('actions.cancel')}
            </button>
            <button type="button"
